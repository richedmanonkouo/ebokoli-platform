...
try {
  ...
  switch ($_GET['view']) {
    case '':
      ...
      /* mibikisi */
      $get_mibikisi = $db->query("SELECT COUNT(*) as count FROM `mibikisi`") or _error('SQL_ERROR');
      $insights['mibikisi'] = $get_mibikisi->fetch_assoc()['count'];
      /* notifications */
      ...
      for ($i = 1; $i <= 12; $i++) {
        ...
        /* get mibikisi */
        $get_monthly_mibikisi = $db->query("SELECT COUNT(*) as count FROM `mibikisi` WHERE YEAR(group_date) = YEAR(CURRENT_DATE()) AND MONTH(group_date) = $i");
        $chart['mibikisi'][$i] = $get_monthly_mibikisi->fetch_assoc()['count'];
        /* get events */
    ...
    case 'mibikisi':
      ...
      switch ($_GET['sub_view']) {
        case '':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Mibikisi"));

          // get data
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query("SELECT COUNT(*) as count FROM `mibikisi` INNER JOIN users ON `mibikisi`.group_admin = users.user_id") or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
          $params['url'] = $system['system_url'] . '/' . $control_panel['url'] . '/mibikisi?page=%s';
          $pager = new Pager($params);
          $limit_query = $pager->getLimitSql();
          $get_rows = $db->query("SELECT `mibikisi`.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM `mibikisi` INNER JOIN users ON `mibikisi`.group_admin = users.user_id ORDER BY `mibikisi`.group_id ASC " . $limit_query) or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['group_picture'] = get_picture($row['group_picture'], 'mobikisi');
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
              $rows[] = $row;
            }
          }
        ...
        case 'find':
          ...
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query(sprintf('SELECT COUNT(*) as count FROM `mibikisi` INNER JOIN users ON `mibikisi`.group_admin = users.user_id WHERE `mibikisi`.group_name LIKE %1$s OR `mibikisi`.group_title LIKE %1$s', secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
          $params['url'] = $system['system_url'] . '/' . $control_panel['url'] . '/mibikisi/find?query=' . $_GET['query'] . '&page=%s';
          $pager = new Pager($params);
          $limit_query = $pager->getLimitSql();
          $get_rows = $db->query(sprintf('SELECT `mibikisi`.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM `mibikisi` INNER JOIN users ON `mibikisi`.group_admin = users.user_id WHERE `mibikisi`.group_name LIKE %1$s OR `mibikisi`.group_title LIKE %1$s ' . $limit_query, secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['group_picture'] = get_picture($row['group_picture'], 'mobikisi');
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
        ...
        case 'edit_group':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT `mibikisi`.*, users.* FROM `mibikisi` INNER JOIN users ON `mibikisi`.group_admin = users.user_id WHERE `mibikisi`.group_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
          $data['group_picture'] = get_picture($data['group_picture'], 'mobikisi');
          $data['user_picture'] = get_picture($data['user_picture'], $data['user_gender']);
          /* get categories */
          $data['categories'] = $user->get_categories("mibikisi_categories");
          /* get custom fields */
          $smarty->assign('custom_fields', $user->get_custom_fields(array("for" => "mobikisi", "get" => "settings", "node_id" => $_GET['id'])));
          /* prepare monetization */
          ...
          if ($data['can_monetize_content']) {
            // get monetozaion plans
            $smarty->assign('monetization_plans', $user->get_monetization_plans($data['group_id'], 'mobikisi'));
          }
          ...
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Mibikisi") . " &rsaquo; " . $data['group_title']);
          break;

        case 'categories':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Mibikisi") . " &rsaquo; " . __("Categories"));

          // get data
          $rows = $user->get_categories("mibikisi_categories");
        ...
        case 'edit_category':
          ...
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Mibikisi") . " &rsaquo; " . __("Categories") . " &rsaquo; " . $data['category_name']);
          break;

        case 'add_category':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Mibikisi") . " &rsaquo; " . __("Categories") . " &rsaquo; " . __("Add New Category"));

          // get data
          $categories = $user->get_categories("mibikisi_categories");
    ...
    case 'wallet':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'savings':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Wallet") . " &rsaquo; " . __("Savings Management"));

          // get user filter
          $where_query = "";
          if (isset($_GET['user_id']) && is_numeric($_GET['user_id'])) {
            $where_query = " WHERE wallet_savings.user_id = " . secure($_GET['user_id'], 'int');
          }

          // get data
          $get_rows = $db->query("SELECT wallet_savings.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, users.user_wallet_balance FROM wallet_savings INNER JOIN users ON wallet_savings.user_id = users.user_id" . $where_query . " ORDER BY wallet_savings.created_at DESC") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
              // Calculate accumulated interest
              $days = (strtotime('now') - strtotime($row['start_date'])) / 86400;
              $interest = ($row['amount'] * $row['interest_rate'] * $days) / (365 * 100);
              $row['current_value'] = $row['amount'] + $interest;
              $row['accumulated_interest'] = $interest;
              $rows[] = $row;
            }
          }
        ...
        case 'loans':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Wallet") . " &rsaquo; " . __("Loans Management"));

          // get user filter
          $where_query = "";
          if (isset($_GET['user_id']) && is_numeric($_GET['user_id'])) {
            $where_query = " WHERE wallet_loans.user_id = " . secure($_GET['user_id'], 'int');
          }

          // get data
          $get_rows = $db->query("SELECT wallet_loans.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, users.user_wallet_balance FROM wallet_loans INNER JOIN users ON wallet_loans.user_id = users.user_id" . $where_query . " ORDER BY wallet_loans.created_at DESC") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
              $rows[] = $row;
        ...
        case 'user_wallet':
          ...
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Wallet") . " &rsaquo; " . __("User Wallet Management"));

          // get user data
          $get_user = $db->query(sprintf("SELECT * FROM users WHERE user_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          ...
          // get user savings
          $get_savings = $db->query(sprintf("SELECT * FROM wallet_savings WHERE user_id = %s ORDER BY created_at DESC", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_savings->num_rows > 0) {
            while ($saving = $get_savings->fetch_assoc()) {
              // Calculate accumulated interest
              $days = (strtotime('now') - strtotime($saving['start_date'])) / 86400;
              $interest = ($saving['amount'] * $saving['interest_rate'] * $days) / (365 * 100);
              $saving['current_value'] = $saving['amount'] + $interest;
              $saving['accumulated_interest'] = $interest;
              $savings[] = $saving;
            }
          }

          // get user loans
          $get_loans = $db->query(sprintf("SELECT * FROM wallet_loans WHERE user_id = %s ORDER BY created_at DESC", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          ...
          // get transactions
          $get_transactions = $db->query(sprintf("SELECT * FROM wallet_transactions WHERE user_id = %s AND (type = 'savings' OR type = 'loans') ORDER BY date DESC LIMIT 20", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          ...
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              ...
              if ($row['node_type'] == "user") {
                ...
                $get_node = $db->query(sprintf("SELECT group_name, group_title, group_picture FROM `mibikisi` WHERE group_id = %s", secure($row['node_id'], 'int'))) or _error('SQL_ERROR');
                if ($get_node->num_rows == 0) continue;
                $node = $get_node->fetch_assoc();
    ...
    case 'tools':
      ...
      switch ($_GET['sub_view']) {
        case 'faker':
          ...
          // get groups categories
          $smarty->assign('mibikisi_categories', $user->get_categories("mibikisi_categories"));
          break;
