...
class User
{
  ...
  /**
   * admin_add_saving
   * Admin ajoute une épargne pour un utilisateur
   */
  public function admin_add_saving($user_id, $amount, $interest_rate = 0, $maturity_date = null, $description = '')
  {
    global $db, $date;
    ...
    $db->query(sprintf('UPDATE users SET user_wallet_balance = user_wallet_balance + %1$s WHERE user_id = %2$s', secure($amount, 'float'), secure($user_id, 'int'))) or _error('SQL_ERROR_THROWEN');
  ...
  /**
   * admin_create_loan
   * Admin crée un emprunt pour un utilisateur
   */
  public function admin_create_loan($user_id, $amount, $interest_rate, $duration_months, $description = '')
  {
    global $db, $date;
  ...
  /**
   * admin_record_loan_payment
   * Admin enregistre un paiement de prêt
   */
  public function admin_record_loan_payment($loan_id, $user_id, $amount)
  {
    global $db, $date;
    ...
    $get_loan = $db->query(sprintf("SELECT * FROM wallet_loans WHERE loan_id = %s AND user_id = %s AND status = 'active'", secure($loan_id, 'int'), secure($user_id, 'int'))) or _error('SQL_ERROR_THROWEN');
   ...
   * Compte les paiements d'un emprunt
   */
