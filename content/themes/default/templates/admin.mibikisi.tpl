<div class="card">
  <div class="card-header with-icon">
    {if $sub_view == "find"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/mibikisi" class="btn btn-md btn-light">
          <i class="fa fa-arrow-circle-left mr5"></i>{__("Go Back")}
        </a>
      </div>
    {elseif $sub_view == "edit_mobikisi"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/mibikisi" class="btn btn-md btn-light">
          <i class="fa fa-arrow-circle-left"></i><span class="ml5 d-none d-lg-inline-block">{__("Go Back")}</span>
        </a>
        <a target="_blank" href="{$system['system_url']}/mibikisi/{$data['group_name']}" class="btn btn-md btn-info">
          <i class="fa fa-eye"></i><span class="ml5 d-none d-lg-inline-block">{__("View Mobikisi")}</span>
        </a>
      </div>
    {elseif $sub_view == "categories"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/mibikisi/add_category" class="btn btn-md btn-primary">
    ...
    {elseif $sub_view == "add_category" || $sub_view == "edit_category"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/mibikisi/categories" class="btn btn-md btn-light">
          <i class="fa fa-arrow-circle-left"></i><span class="ml5 d-none d-lg-inline-block">{__("Go Back")}</span>
        </a>
      </div>
    {/if}
    <i class="fa fa-users mr10"></i>{__("Mibikisi")}
    {if $sub_view == "find"} &rsaquo; {__("Find")}{/if}
    {if $sub_view == "edit_mobikisi"} &rsaquo; {$data['group_title']}{/if}
  ...
  {if $sub_view == "" || $sub_view == "find"}
    ...
    <div class="card-body">
      ...
      <!-- search form -->
      <div class="mb20">
        <form class="d-flex flex-row align-items-center flex-wrap" action="{$system['system_url']}/{$control_panel['url']}/mibikisi/find" method="get">
        ...
        <div class="form-text small">
          {__('Search by Mobikisi Web Address or Title')}
        </div>
      </div>
      <!-- search form -->
          ...
          <tbody>
            {if $rows}
              {foreach $rows as $row}
                <tr>
                  <td>
                    <a href="{$system['system_url']}/mibikisi/{$row['group_name']}" target="_blank">{$row['group_id']}</a>
                  </td>
                  <td>
                    <a target="_blank" href="{$system['system_url']}/mibikisi/{$row['group_name']}">
                  ...
                  <td>
                    <a data-bs-toggle="tooltip" title='{__("Edit")}' href="{$system['system_url']}/{$control_panel['url']}/mibikisi/edit_mobikisi/{$row['group_id']}" class="btn btn-sm btn-icon btn-rounded btn-primary">
    ...
    <div class="card-body">
      ...
      <div class="tab-content">
        ...
        <div class="tab-pane active" id="group_settings">
          <form class="js_ajax-forms" data-url="admin/mibikisi.php?do=edit_mobikisi&edit=settings&id={$data['group_id']}">
            ...
            <div class="row form-group">
              ...
              <div class="col-md-9">
                <select class="form-select" name="privacy">
                  <option {if $data['group_privacy'] == "public"}selected{/if} value="public">{__("Public Mobikisi")}</option>
                  <option {if $data['group_privacy'] == "closed"}selected{/if} value="closed">{__("Closed Mobikisi")}</option>
                  <option {if $data['group_privacy'] == "secret"}selected{/if} value="secret">{__("Secret Mobikisi")}</option>
                </select>
                <div class="form-text">
            ...
            <div class="form-table-row">
              ...
              <div>
                ...
                <div class="form-text d-none d-sm-block">{__("Enable chat box for this mobikisi")}</div>
              </div>
              <div class="text-end">
            ...
            <div class="form-table-row">
              <div>
                ...
                <div class="form-text d-none d-sm-block">
                  {__("All posts must be approved by a mobikisi admin")}<br>
            ...
            <div class="card-footer-fake text-end">
              ...
              <button type="button" class="btn btn-danger js_admin-deleter" data-handle="group" data-id="{$data['group_id']}" data-redirect="{$system['system_url']}/{$control_panel['url']}/mibikisi">
                <i class="fa fa-trash-alt mr5"></i>{__("Delete Mobikisi")}
              </button>
        ...
        <div class="tab-pane" id="group_monetization">
          {if $data['can_monetize_content']}
            <form class="js_ajax-forms" data-url="admin/mibikisi.php?do=edit_mobikisi&edit=monetization&id={$data['group_id']}">
              <div class="form-table-row">
                <div class="avatar">
              ...
              <div class="row form-group">
                ...
                <div class="col-md-9">
                  <div class="payment-plans">
                    ...
                    {/foreach}
                    <div data-toggle="modal" data-url="monetization/controller.php?do=add&node_id={$data['group_id']}&node_type=mobikisi" class="payment-plan new">{__("Add new plan")} </div>
                  </div>
                </div>
              </div>
      ...
      <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover js_treegrid">
          ...
          <tbody>
            {if $rows}
              {foreach $rows as $row}
                {include file='__categories.recursive_rows.tpl' _url="mibikisi" _handle="group_category"}
              {/foreach}
            {else}
              <tr>
  ...
  {elseif $sub_view == "edit_category"}
    ...
    <form class="js_ajax-forms" data-url="admin/mibikisi.php?do=edit_category&id={$data['category_id']}">
      <div class="card-body">
        <div class="row form-group">
          <label class="col-md-3 form-label">
            {__("Name")}
