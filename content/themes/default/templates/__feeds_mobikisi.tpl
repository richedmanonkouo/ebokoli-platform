{if $_tpl == "box"}
  <li class="col-md-6 col-lg-3">
    <div class="ui-box {if $_darker}darker{/if}">
      <div class="img">
        <a href="{$system['system_url']}/mibikisi/{$_group['group_name']}{if $_search}?ref=qs{/if}">
          <img alt="{$_group['group_title']}" src="{$_group['group_picture']}" />
        </a>
      </div>
      <div class="mt10">
        <a class="h6" href="{$system['system_url']}/mibikisi/{$_group['group_name']}{if $_search}?ref=qs{/if}">{$_group['group_title']}</a>
...
{elseif $_tpl == "list"}
  <li class="feeds-item">
    <div class="data-container {if $_small}small{/if}">
      <a class="data-avatar" href="{$system['system_url']}/mibikisi/{$_group['group_name']}{if $_search}?ref=qs{/if}">
      ...
      <div class="data-content">
        ...
        <div>
          <span class="name">
            <a href="{$system['system_url']}/mibikisi/{$_group['group_name']}{if $_search}?ref=qs{/if}">{$_group['group_title']}</a>
          </span>
          <div>{$_group['group_members']} {__("Members")}</div>
        </div>
      </div>
