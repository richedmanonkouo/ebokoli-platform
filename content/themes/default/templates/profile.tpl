  ...
  <div class="row">
    ...
    <div class="col-12 sg-offcanvas-mainbar">
      ...
      <div class="profile-tabs-wrapper d-flex justify-content-evenly">
        ...
        {/if}
        {if $system['mibikisi_enabled']}
          <a href="{$system['system_url']}/{$profile['user_name']}/mibikisi" {if $view == "mibikisi"}class="active" {/if}>
            {include file='__svg_icons.tpl' icon="group" class="main-icon mr5" width="24px" height="24px"}
            <span class="ml5 d-none d-xxl-inline-block">{__("Mibikisi")}</span>
          </a>
        {/if}        
      ...
      <div class="row">
        ...
        {if $view == ""}
          ...
          <div class="col-lg-4 order-2 order-lg-1">
            ...
            {if $user->_logged_in && $user->_data['user_id'] != $profile['user_id'] && $profile['can_receive_tips'] && $profile['user_tips_enabled']}
              <div class="d-grid">
                <button type="button" class="btn bg-red rounded-pill mb20" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$profile['user_id']}"}'>
            ...
            <!-- mibikisi -->
            {if $system['mibikisi_enabled'] && count($profile['mibikisi']) > 0}
              <div class="card">
                <div class="card-header bg-transparent">
                  {include file='__svg_icons.tpl' icon="group" class="main-icon mr5" width="24px" height="24px"}
                  <strong><a href="{$system['system_url']}/{$profile['user_name']}/mibikisi">{__("Mibikisi")}</a></strong>
                </div>
                <div class="card-body ptb10 plr10">
                  <div class="row">
                    {foreach $profile['mibikisi'] as $_group}
                      <div class="col-3 col-lg-4">
                        <div class="circled-user-box">
                          <a class="user-box" href="{$system['system_url']}/mibikisi/{$_group['group_name']}">
                            <img alt="{$_group['group_title']}" src="{$_group['group_picture']}" />
                            <div class="name" title="{$_group['group_title']}">
                              {$_group['group_title']}
                            </div>
        ...
        {elseif $view == "mibikisi"}
          <!-- mibikisi -->
          <div class="col-12">
            <div class="card">
              <div class="card-header with-icon">
                <!-- panel title -->
                {include file='__svg_icons.tpl' icon="group" class="main-icon mr10" width="24px" height="24px"}
                {__("Mibikisi")}
                <!-- panel title -->
              </div>
              <div class="card-body">
                {if $profile['user_id'] == $user->_data['user_id'] || $profile['user_privacy_mibikisi'] == "public" || ($profile['user_privacy_mibikisi'] == "friends" && $profile['we_friends'])}
                  {if count($profile['mibikisi']) > 0}
                    <ul class="row">
                      {foreach $profile['mibikisi'] as $_group}
                        {include file='__feeds_group.tpl' _tpl="box" _darker=true}
                      {/foreach}
                    ...
                    {if count($profile['mibikisi']) >= $system['max_results_even']}
                      <!-- see-more -->
                      <div class="alert alert-post see-more js_see-more" data-get="profile_groups" data-uid="{$profile['user_id']}">
                        <span>{__("See More")}</span>
                        <div class="loader loader_small x-hidden"></div>
