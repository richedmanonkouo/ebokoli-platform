...
try {
  ...
  $get_group = $db->query(sprintf("SELECT `mibikisi`.*, picture_photo.source as group_picture_full, cover_photo.source as group_cover_full, mibikisi_categories.category_name as group_category_name FROM `mibikisi` LEFT JOIN posts_photos as picture_photo ON groups.group_picture_id = picture_photo.photo_id LEFT JOIN posts_photos as cover_photo ON `mibikisi`.group_cover_id = cover_photo.photo_id LEFT JOIN mibikisi_categories ON `mibikisi`.group_category = mibikisi_categories.category_id WHERE `mibikisi`.group_name = %s", secure($_GET['username']))) or _error('SQL_ERROR_THROWEN');
  ...
  if (strtolower($_GET['username']) == strtolower($group['group_name']) && $_GET['username'] != $group['group_name']) {
    redirect('/mibikisi/' . $group['group_name']);
  }
  /* get group picture */
  $group['group_picture_default'] = ($group['group_picture']) ? false : true;
  $group['group_picture'] = get_picture($group['group_picture'], 'mobikisi');
  ...
  $group['posts_count'] = $user->get_posts_count($group['group_id'], 'mobikisi');
  /* get group photos count */
  $group['photos_count'] = $user->get_photos_count($group['group_id'], 'mobikisi');
  /* get group videos count */
  if ($system['videos_enabled']) {
    $group['videos_count'] = $user->get_videos_count($group['group_id'], 'mobikisi');
  }
  ...
  if ($group['has_subscriptions_plans']) {
    if ($user->_logged_in) {
      if ($user->_data['user_group'] == 3 && !$group['i_admin']) {
        if (!$user->is_subscribed($group['group_id'], 'mobikisi')) {
  ...
  switch ($_GET['view']) {
    case '':
      /* get custom fields */
      $smarty->assign('custom_fields', $user->get_custom_fields(array("for" => "mobikisi", "get" => "profile", "node_id" => $group['group_id'])));

      /* get subscribers */
      if ($group['has_subscriptions_plans']) {
        /* get subscribers count */
        $group['subscribers_count'] = $user->get_subscribers_count($group['group_id'], 'mobikisi');
        /* get subscribers */
        if ($group['subscribers_count'] > 0) {
          $group['subscribers'] = $user->get_subscribers($group['group_id'], 'mobikisi');
        }
      }
      ...
      if (!$group['needs_subscription']) {
        $group['photos'] = $user->get_photos($group['group_id'], 'mobikisi');
      }
      ...
      /* get ads */
      $smarty->assign('ads', $user->ads('mibikisi', $group['group_id']));
      break;
    ...
    case 'albums':
      ...
      if (!$group['needs_subscription']) {
        /* get albums */
        $group['albums'] = $user->get_albums($group['group_id'], 'mobikisi');
      }
      break;
    ...
    case 'videos':
      ...
      if (!$group['needs_subscription']) {
        /* get videos */
        $group['videos'] = $user->get_videos($group['group_id'], 'mobikisi');
      }
      break;
    ...
    case 'settings':
      ...
      switch ($sub_view) {
        case '':
          /* get custom fields */
          $smarty->assign('custom_fields', $user->get_custom_fields(array("for" => "mobikisi", "get" => "settings", "node_id" => $group['group_id'])));
        ...
        case 'monetization':
          ...
          $smarty->assign('monetization_plans', $user->get_monetization_plans($group['group_id'], 'mobikisi'));

          // get subscribers count
          $smarty->assign('subscribers_count', $user->get_subscribers_count($group['group_id'], 'mobikisi'));
          break;
  ...
  if (isset($_GET['ref']) && $_GET['ref'] == "qs") {
    $user->set_search_log($group['group_id'], 'mobikisi');
  }
} catch (Exception $e) {
...
// assign variables
$smarty->assign('mobikisi', $group);
$smarty->assign('view', $_GET['view']);

// page footer
page_footer('mobikisi');
