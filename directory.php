...
try {
  ...
  switch ($_GET['view']) {
    ...
    case 'mibikisi':
      ...
      // page header
      page_header(__("Mibikisi Directory") . ' | ' . __($system['system_title']), __($system['system_description_directory']));

      // pager config
      require('includes/class-pager.php');
      $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
      $total = $db->query("SELECT COUNT(*) as count FROM `mibikisi` WHERE group_privacy = 'public'") or _error('SQL_ERROR');
      $params['total_items'] = $total->fetch_assoc()['count'];
      $params['items_per_page'] = $system['max_results'];
      $params['url'] = $system['system_url'] . '/directory/' . 'mibikisi' . '/%s';
      $pager = new Pager($params);
      $limit_query = $pager->getLimitSql();

      // get groups
      $rows = [];
      $get_rows = $db->query("SELECT * FROM `mibikisi` WHERE group_privacy = 'public' " . $limit_query) or _error('SQL_ERROR');
      while ($row = $get_rows->fetch_assoc()) {
